...
#define MAX_CODIGO_RESERVA sizeof(char)*65535 /* máximo possível para códigos de reserva */
...
Aeroporto _aeroportos[40];	/* vetor de aeroportos */

int _numVoos = 0		/* número de voos introduzidos */;
Voo _voos[30000] /* vetor de voos */;

int numReservas = 0 /* número de reservas introduzidas */;
Reserva *reservas;
...
void listaVoosPartida() {
	...
	for (i = 0; i < n; i++)
		if (converteDataNum(_voos[indexVoos[i]].data) == 
		converteDataNum(_hoje))
...
/* conjunto de verificações relativas às reservas */

int eh_id_reserva_valido(char * id){
	int i, l = strlen(id);
	if (l < 10)
		return FALSE;
	for (i = 0; i < l; ++i)
...
int reserva_existe(Reserva r){
	int i;
	for (i = 0; i < numReservas; ++i)
		if (strcmp(reservas[i].id_reserva,r.id_reserva) == 0)
			return TRUE;
	return FALSE;
}

int voo_existe(Reserva r){
    int i;
    for (i = 0; i < _numVoos; ++i)
        if (strcmp(r.id,_voos[i].id) == 0 && 
        converteDataNum(r.data) == converteDataNum(_voos[i].data))
            return TRUE;
    return FALSE;
}
int excede_voo(Reserva r){
	int i;
	for (i = 0; i < _numVoos; ++i){
		if ((strcmp(_voos[i].id,r.id) == 0) && (converteDataNum(_voos[i].data) 
        == converteDataNum(r.data))){
			if (_voos[i].ocupados + r.num_passageiros > _voos[i].capacidade)
				return TRUE;
            else
                _voos[i].ocupados += r.num_passageiros;
            break;
        }
	}
	return FALSE;
...
int eh_reserva_valida(Reserva r){
	if (!eh_id_reserva_valido(r.id_reserva))
		printf("invalid reservation code\n");
	else if (!voo_existe(r))
		printf("%s: flight does not exist\n",r.id);
	else if (reserva_existe(r))
		printf("%s: flight reservation already used\n",r.id_reserva);
	else if (excede_voo(r))
		printf("too many reservations\n");
	else if (!validaData(r.data))
		printf("invalid date\n");
	else if (r.num_passageiros <= 0)
...
/* Funções que servem para mostrar as reservas quando tal é adequado */

void mostra_reserva(Reserva r){
	printf("%s %d\n",r.id_reserva,r.num_passageiros);
}

int rescmp(int i, int j){
	if (strcmp(reservas[i].id_reserva,reservas[j].id_reserva) > 0)
		return TRUE;
	return FALSE;
}

void mostra_reservas_voo(Reserva r){
	int i,*indices = (int*) malloc(sizeof(int)*numReservas),j = 0;
	for(i = 0; i < numReservas; ++i)
		if ((strcmp(r.id,reservas[i].id) == 0) && 
        (converteDataNum(r.data) == converteDataNum(reservas[i].data)))
			indices[j++] = i;
	bubbleSort(indices, j, rescmp);
	for (i = 0; i < j; ++i)
		mostra_reserva(reservas[indices[i]]);
}

/* Função que cria as reservas e as adiciona ao vetro de reservas */

void reserva(){
	Reserva r, *link;
	leProximaPalavra(r.id);
	r.data = leData();
	r.id_reserva = (char*) malloc(MAX_CODIGO_RESERVA);
	if (r.id_reserva == NULL){
		printf("No memory");
		exit(1);
	}
	if(!leProximaPalavra(r.id_reserva))
        scanf("%d\n",&(r.num_passageiros));
	r.id_reserva = (char*) realloc(r.id_reserva,
	sizeof(char)*(strlen(r.id_reserva) + 1));
	if (strlen(r.id_reserva) == 0){
        if (!voo_existe(r))
            printf("%s: flight does not exist\n",r.id);        
        else if (!validaData(r.data))
            printf("invalid date\n");
        else
		    mostra_reservas_voo(r);
        return;
    }
	if (!eh_reserva_valida(r))
		return;
	link = reservas;
	reservas = (Reserva*) realloc(reservas,sizeof(Reserva)*(++numReservas));
	if (reservas == NULL){
		printf("No memory\n");
		free(link);
		exit(1);
...
/* Função que elimina os voos e reservas */

void limpa(){
	char * id;
	int i, passo = 0, existe = 0;
    id = (char*) malloc(MAX_CODIGO_RESERVA);
	if (id == NULL){
		printf("No memory\n");
		exit(1);
	}
	...
	if (eh_id_reserva_valido(id))
		for (i = 0;i + passo < numReservas; ++i){
			if (i + passo < numReservas 
			&& strcmp(reservas[i + passo].id_reserva,id) == 0){
				_voos[encontraVoo(reservas[i + passo].id,
				reservas[i + passo].data)].ocupados -= 
				reservas[i + passo].num_passageiros;
				existe = TRUE;
				++passo;
			}
			if (i + passo < numReservas)
				reservas[i] = reservas[i + passo];
		}
	numReservas -= passo;
	reservas = (Reserva*) realloc(reservas,sizeof(Reserva)*numReservas);
	passo = 0;
	if (validaIDVoo(id)){
		for (i = 0;i + passo < _numVoos; ++i){
			while (i + passo < _numVoos && 
			strcmp(_voos[i + passo].id,id) == 0
			&& converteDataNum(_voos[i + passo].data) <
			converteDataNum(_hoje)){
				++passo;
				existe = TRUE;
			}
			if (i + passo < _numVoos)
				_voos[i] = _voos[i + passo];
		}
		_numVoos -= passo;
		passo = 0;
		for (i = 0;i + passo < numReservas; ++i){
			while (i + passo < numReservas 
			&& strcmp(reservas[i + passo].id,id) == 0 &&
			converteDataNum(reservas[i + passo].data) <
			converteDataNum(_hoje)){
				++passo;
			}
			if (i + passo < numReservas)
...
/* Função main, serve apenas para ler os comandos e chamar as funções apropriadas */

int main() {
	int c;
	reservas = malloc(sizeof(reservas));
	while ((c = getchar()) != EOF) {
		switch (c) {
		case 'q': free(reservas); 
			return 0;
